 ...
 */
 class ListsSuite {
   ...
   */
  @Test def `test sum of basic list`: Unit = {
    assert(sum(List(1,2,0)) == 3)
  }

  @Test def `test sum of empty list`: Unit = {
    assert(sum(List()) == 0)
  }

  @Test def `test sum of single elem list`: Unit = {
    assert(sum(List(1)) == 1)
  }

  @Test def `test sum of negative numbers`: Unit = {
    assert(sum(List(-1, 1)) == 0)
  }

  @Test def `test sum of slightly longer list`: Unit = {
    assert(sum(List(-1, 1, 5, 10)) == 15)
  }

  @Test def `test max of simple list`: Unit = {
    assert(max(List(3, 7, 2)) == 7)
  }

  @Test def `test max of list with negatives`: Unit = {
    assert(max(List(3, -7, -2)) == 3)
  }

  @Test def `test max of empty list`: Unit = {
    try {
      max(List())
      Assert.fail("No exception thrown for max of empty list")
    } catch {
      case e: NoSuchElementException => ()
    }
  }

  @Test def `test max of single elem list`: Unit = {
    assert(max(List(7)) == 7)
  }
